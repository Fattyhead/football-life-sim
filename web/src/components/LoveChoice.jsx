...
/* 求婚／出軌誘惑／狗仔自動觸發——戀愛常駐事件(見 flow/romance.js)裡
   ...
   稽核修正(使用者委託：繁中/簡中雙語)：跟 TrainingRivalry.jsx 同一套
   手法——插值(對象姓名/頭銜)先在 JS 端組成完整字串，最後整句只呼叫一次
   t()，不逐段包。 */
export default function LoveChoice({ S, pending, onPick }) {
  ...
  if (pending.type === 'PAPARAZZI') {
    const titleClause = pending.partner.title ? `（${pending.partner.title}）` : '';
    const message = `狗仔拍到你跟${pending.partner.name}${titleClause}一起出遊的照片，追著問你要不要承認——這段關係，你想公開嗎？`;
    return (
      <div className="card love-choice">
        ...
        <div className="option-grid">
          <button className="option-btn" onClick={() => onPick('admit')}>
            <span className="opt-label">{t('承認')}</span>
            <span className="opt-desc">{t('大方承認，你們正式開始交往。')}</span>
          </button>
          <button className="option-btn" onClick={() => onPick('deny')}>
            <span className="opt-label">{t('否認')}</span>
            <span className="opt-desc">{t('這次先算了，沒有任何代價，之後還可能遇到類似的機會。')}</span>
          </button>
        </div>
      </div>
    );
  ...
  if (pending.type === 'PROPOSE') {
    ...
    const titleClause = partnerTitle ? `（${partnerTitle}）` : '';
    const message = `你看著身邊的${partnerName}${titleClause}，忽然很確定——是不是該把這件事說出口了？`;
    return (
      <div className="card love-choice">
        <p className="eyebrow">{t('人生的抉擇')}</p>
        <p className="frame-text">{t(message)}</p>
        ...
        <div className="option-grid">
          <button className="option-btn" onClick={() => onPick('propose')}>
            <span className="opt-label">{t('求婚')}</span>
            <span className="opt-desc">{t('說出口，但不是每次都會如你所願——對方也有可能拒絕。')}</span>
          </button>
          <button className="option-btn" onClick={() => onPick('wait')}>
            <span className="opt-label">{t('再等等')}</span>
            <span className="opt-desc">{t('關係繼續，但拖越久，這段感情因為遲遲沒有進展而分手的風險越高。')}</span>
          </button>
        </div>
      </div>
    );
  ...
  return (
    <div className="card love-choice">
      <p className="eyebrow">{t('人生的抉擇')}</p>
      <p className="frame-text">{t('聚會上，有人若有似無地靠近，遞來一個心照不宣的眼神——這一步，要不要跨過去？')}</p>
      {riskFlavor && <p className="streak-flavor">{t(riskFlavor)}</p>}
      <div className="option-grid">
        <button className="option-btn" onClick={() => onPick('accept')}>
          <span className="opt-label">{t('接受')}</span>
          <span className="opt-desc">{t('有機會藏得住，但也有可能被抓包，付出的代價會重得多。')}</span>
        </button>
        <button className="option-btn" onClick={() => onPick('decline')}>
          <span className="opt-label">{t('拒絕')}</span>
          <span className="opt-desc">{t('安全，不留下任何痕跡。')}</span>
        </button>
      </div>
    </div>
  );
