...
/* 國家隊隱藏對手線的 CROSSROADS(個人表現/團隊優先)——只有真正的世界盃年
   ...
   稽核修正(使用者委託：繁中/簡中雙語)：跟其他抉擇卡片同一套手法。 */
export default function NationalRivalChoice({ S, onPick }) {
  const t = useT();
  const rival = S.nationalRival;
  if (!rival) return null;
  const message = `世界盃在即，備戰營裡，你跟${rival.name}都想證明自己——這次，要把這屆賽事當成個人的舞台，還是把心力放在整支球隊身上？`;
    ...
    <div className="card love-choice">
      ...
      <div className="option-grid">
        <button className="option-btn" onClick={() => onPick('compete')}>
          <span className="opt-label">{t('個人表現')}</span>
          <span className="opt-desc">{t('全力證明自己，這屆世界盃的個人數據會明顯提升，但不會幫到球隊的晉級機率。')}</span>
        </button>
        <button className="option-btn" onClick={() => onPick('teamFocus')}>
          <span className="opt-label">{t('團隊優先')}</span>
          <span className="opt-desc">{t('把心力放在整支球隊的備戰上，明顯提升這屆的晉級機率，沒有個人數據的額外加成。')}</span>
        </button>
      </div>
    </div>
  );
