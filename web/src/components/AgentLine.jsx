...
export default function AgentLine({ S, pending, onPick }) {
  ...
  if (pending.type === 'ENCOUNTER') {
    ...
    const message = `一位經紀人主動聯繫你——${c.title}・${c.name}，他看好你的潛力，想成為你的代理人。你要接受，還是婉拒？`;
    return (
      <div className="card love-choice">
        ...
        <div className="option-grid">
          <button className="option-btn" onClick={() => onPick('accept')}>
            ...
            <span className="opt-desc">{t('簽下這位經紀人，之後生涯的談判/交易會有更多互動。')}</span>
          </button>
  ...
  const message = `經紀人${agent.title}・${agent.name}捎來消息：有一筆更大膽的操作可以談，也可以照原本的節奏穩紮穩打——你怎麼決定？`;
    ...
    <div className="card love-choice">
      ...
      <div className="option-grid">
        <button className="option-btn" onClick={() => onPick('bold')}>
          <span className="opt-label">{t('大膽操作')}</span>
          <span className="opt-desc">{t('轉會買氣立即拉高，但跟俱樂部的關係會跟著繃緊一點。')}</span>
        </button>
        <button className="option-btn" onClick={() => onPick('steady')}>
          <span className="opt-label">{t('穩紮穩打')}</span>
          <span className="opt-desc">{t('薪資溢價與合約風險折扣都往上疊，沒有額外曝光度。')}</span>
        </button>
      </div>
    </div>
  );
