...
export default function CreateScreen({ onCreate }) {
  ...
  return (
    <div className="app-shell">
      <h1 className="screen-title">{t('創建球員')}</h1>
      <div className="card">
        <div className="field-row">
          <label htmlFor="name">{t('姓名')}</label>
          <input id="name" type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder={t('無名小將')} />
        </div>
        <div className="field-row">
          <label htmlFor="jersey">{t('背號')}</label>
          <input id="jersey" type="text" inputMode="numeric" value={jersey} onChange={(e) => setJersey(e.target.value)} />
        </div>
        <div className="field-row">
          <label htmlFor="pos">{t('位置')}</label>
        ...
        <div className="field-row">
          <label htmlFor="region">{t('出身地區')}</label>
