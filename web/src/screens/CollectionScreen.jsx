  ...
  function handleImportFile(e) {
    ...
    reader.onload = () => {
      try {
        ...
        setImportStatus('備份已匯入並合併進現有紀錄，不會覆蓋掉這段時間新拿到的成就。');
      } catch {
        setImportStatus('這個檔案看起來不是足球人生模擬器的備份檔，匯入失敗——你原本的紀錄沒有受影響。');
      }
    };
    reader.onerror = () => setImportStatus('讀取檔案失敗，匯入沒有生效。');
    reader.readAsText(file);
  }

  return (
    <div className="app-shell">
      <h1 className="screen-title">成就典藏</h1>
      <div className="card">
        <p className="frame-text" style={{ margin: 0 }}>
          這裡累積的是你玩過的<b>每一局</b>生涯——不管換過幾次種子，拿到過的徽章/稱號都會留在這裡，不會因為開新生涯而重置。
        </p>
      </div>

      <div className="card">
        <p className="eyebrow">生涯總覽</p>
        <div className="collection-stats-row">
          <div className="collection-stat">
            <div className="collection-stat-num">{collection.careersPlayed}</div>
            <div className="collection-stat-lbl">玩過幾局</div>
          </div>
          <div className="collection-stat">
            <div className="collection-stat-num">{collection.bestLegendPercent.toFixed(1)}%</div>
            <div className="collection-stat-lbl">最高傳奇度</div>
          </div>
          <div className="collection-stat">
            <div className="collection-stat-num">{collection.bestTier || '—'}</div>
            <div className="collection-stat-lbl">最高段位</div>
          </div>
        </div>
      </div>
      ...
      <div className="card">
        ...
        <p className="frame-text" style={{ margin: '0 0 12px' }}>
          這份紀錄只存在這台裝置的這個瀏覽器裡，沒有帳號、不會自動同步。想換裝置或怕不小心清掉，先匯出一份備份檔存好；
          之後隨時可以匯入合併回來，不會蓋掉這段時間新拿到的紀錄。
        </p>
